...
background_image = pg.image.load("img/board.png") 
...
# roll and save the value of dice for p1
def dice_value():
...
# roll and save the value of dice for p2
def cpu_dice_value():
    value_1 = random.randint(1, 6)
    value_2 = random.randint(1, 6)
    pg.mixer.Sound.play(move_sound)
    ...
    def receiving_light(self, colour):
        if colour == "white":
            if len(white_reached_home) == 15:
                screen.blit(bearing_off_light, (960, 60))
        else:
            if len(black_reached_home) == 15:
                screen.blit(bearing_off_light, (960, 450))
...
# createblack pieces
black_piece1 = my_piece("black"); black_piece2 = my_piece("black"); black_piece3 = my_piece("black")
black_piece4 = my_piece("black"); black_piece5 = my_piece("black"); black_piece6 = my_piece("black")
black_piece7 = my_piece("black"); black_piece8 = my_piece("black"); black_piece9 = my_piece("black")
black_piece10 = my_piece("black"); black_piece11 = my_piece("black"); black_piece12 = my_piece("black")
black_piece13 = my_piece("black"); black_piece14 = my_piece("black"); black_piece15 = my_piece("black")

# create white pieces
white_piece1 = my_piece("white"); white_piece2 = my_piece("white"); white_piece3 = my_piece("white")
white_piece4 = my_piece("white"); white_piece5 = my_piece("white"); white_piece6 = my_piece("white")
white_piece7 = my_piece("white"); white_piece8 = my_piece("white"); white_piece9 = my_piece("white")
white_piece10 = my_piece("white"); white_piece11 = my_piece("white"); white_piece12 = my_piece("white")
white_piece13 = my_piece("white"); white_piece14 = my_piece("white"); white_piece15 = my_piece("white")


#dice button width = 45px, height = 120px
roll_w_inactive = pg.image.load("img/roll_w_inactive.png")
roll_w_active = pg.image.load("img/roll_w_active.png")

roll_b_inactive = pg.image.load("img/roll_b_inactive.png")
roll_b_active = pg.image.load("img/roll_b_active.png")
# roll_b_active = pg.transform.rotate(roll_b_active, 270)

# blank dice
dice = pg.image.load("img/blank.png")

# stack objects - set as initialized board
stack1 = column_stack(1, black_piece14, black_piece15); stack2 = column_stack(2, None); stack3 = column_stack(3, None)
stack4 = column_stack(4, None); stack5 = column_stack(5, None); stack6 = column_stack(6, white_piece1, white_piece2, white_piece3, white_piece4, white_piece5)
stack7 = column_stack(7, None); stack8 = column_stack(8, white_piece6, white_piece7, white_piece8)
stack9 = column_stack(9, None); stack10 = column_stack(10, None); stack11 = column_stack(11, None)
...
stack14 = column_stack(14, None); stack15 = column_stack(15, None); stack16 = column_stack(16, None)
stack17 = column_stack(17, black_piece8, black_piece7, black_piece6); stack18 = column_stack(18, None)
stack19 = column_stack(19, black_piece5, black_piece4, black_piece3, black_piece2, black_piece1)
stack20 = column_stack(20, None); stack21 = column_stack(21, None); stack22 = column_stack(22, None); stack23 = column_stack(23, None)
stack24 = column_stack(24, white_piece15, white_piece14)

# array/list of stack/column objects
all_stack_list = [
...
# dictionary of all stacks/columns
all_stack_dict = {
    1: stack1, 2: stack2, 3: stack3, 4: stack4, 5: stack5, 6: stack6, 7: stack7, 8: stack8, 9: stack9, 10: stack10,
    11: stack11, 12: stack12, 13: stack13, 14: stack14, 15: stack15, 16: stack16, 17: stack17, 18: stack18,
    19: stack19, 20: stack20, 21: stack21, 22: stack22, 23: stack23, 24: stack24
}
...
while running:
    ...
    for event in pg.event.get():
        ...
        # check turn and highlight playable pucks after dice roll
        if turn == "you" and wxpos <= mouse[0] <= wxpos+bwidth and wypos <= mouse[1] <= wypos+bheight and click[0] == 1: 
                light_white_keys(white_light_pieces)
                you_dice_rolled = True
                light_trigerred = True

        if turn == "cpu" and bxpos <= mouse[0] <= bxpos+bwidth and bypos <= mouse[1] <= bypos+bheight and click[0] == 1:
                light_black_keys(black_light_pieces)
        ...
        if turn == "you":
            for i in white_light_pieces:
                ...
                if event.type == pg.KEYDOWN and (event.key == pg.K_m or event.key == pg.K_SPACE) and you_dice_rolled:
                    ...
                    if click[0] == 1 and i[1].coordinate[0] <= mouse[0] <= i[1].coordinate[0] + 56 and i[1].coordinate[1] <= mouse[1] <= i[1].coordinate[1] + 56:
                        if event.key == pg.K_m:
                            if len(white_reached_home) <= 15:
            ...
            for i in black_light_pieces:
                ...
                if event.type == pg.KEYDOWN and (event.key == pg.K_m or event.key == pg.K_SPACE) and cpu_dice_rolled:
                    if len(my_middle_stack.elements) > 0 and my_middle_stack.elements[-1].id == "black":
                        ...
                        d1, d2 = i[0].location + dice_cpu[0], i[0].location + dice_cpu[1]
                    if click[0] == 1 and i[1].coordinate[0] <= mouse[0] <= i[1].coordinate[0] + 56 and \
                            i[1].coordinate[1] <= mouse[1] <= i[1].coordinate[1] + 56:
                        if event.key == pg.K_m:
                            if len(black_reached_home) <= 15:
    ...
    screen.blit(black_piece1.image, black_piece1.coordinate); screen.blit(black_piece2.image, black_piece2.coordinate)
    screen.blit(black_piece3.image, black_piece3.coordinate); screen.blit(black_piece4.image, black_piece4.coordinate)
    screen.blit(black_piece5.image, black_piece5.coordinate); screen.blit(black_piece6.image, black_piece6.coordinate)
    screen.blit(black_piece7.image, black_piece7.coordinate); screen.blit(black_piece8.image, black_piece8.coordinate)
    screen.blit(black_piece9.image, black_piece9.coordinate); screen.blit(black_piece10.image, black_piece10.coordinate)
    screen.blit(black_piece11.image, black_piece11.coordinate); screen.blit(black_piece12.image, black_piece12.coordinate)
    screen.blit(black_piece13.image, black_piece13.coordinate); screen.blit(black_piece14.image, black_piece14.coordinate)
    screen.blit(black_piece15.image, black_piece15.coordinate)

    screen.blit(white_piece1.image, white_piece1.coordinate); screen.blit(white_piece2.image, white_piece2.coordinate)
    screen.blit(white_piece3.image, white_piece3.coordinate); screen.blit(white_piece4.image, white_piece4.coordinate)
    screen.blit(white_piece5.image, white_piece5.coordinate); screen.blit(white_piece6.image, white_piece6.coordinate)
    screen.blit(white_piece7.image, white_piece7.coordinate); screen.blit(white_piece8.image, white_piece8.coordinate)
    screen.blit(white_piece9.image, white_piece9.coordinate); screen.blit(white_piece10.image, white_piece10.coordinate)
    screen.blit(white_piece11.image, white_piece11.coordinate); screen.blit(white_piece12.image, white_piece12.coordinate)
    screen.blit(white_piece13.image, white_piece13.coordinate); screen.blit(white_piece14.image, white_piece14.coordinate)
    screen.blit(white_piece15.image, white_piece15.coordinate)

    #for human player
    if turn == "you":
        # step 2 (dice rolling)
        if you_dice_rolled == False:
            if wxpos <= mouse[0] <= wxpos+bwidth and wypos <= mouse[1] <= bheight:
                screen.blit(roll_w_active, (wxpos, wypos))

                if click[0] == 1:
                    player1_turn_msg = False
                    dice_value()
            else:
                screen.blit(roll_w_inactive, (wxpos, wypos))
                screen.blit(bearing_off_light, (960, 60)) # REMOVE
                screen.blit(bearing_off_light, (960, 450)) # REMOVE 
        ...
        if you_dice_rolled == True and len(white_light_pieces) > 0:
            ...
            for i in white_light_pieces:
                ...
                if click[0] == 1 and i[1].coordinate[0] <= mouse[0] <= i[1].coordinate[0] + 56 and i[1].coordinate[
                    1] <= mouse[1] <= i[1].coordinate[1] + 56:
    ...
    if turn == "cpu":
        ...
        if cpu_dice_rolled == False:
            if bxpos <= mouse[0] <= bxpos+bwidth and bypos <= mouse[1] <= bypos+bheight:
                screen.blit(roll_b_active, (bxpos, bypos))

                if click[0] == 1:
                    player2_turn_msg = False
                    cpu_dice_value()
            else:
                screen.blit(roll_b_inactive, (bxpos, bypos))
    ...
    screen.blit(dice1.my_dice, (21, 425))
    screen.blit(dice2.my_dice, (21, 485))

    screen.blit(dice1_cpu.my_dice, (21, 30))
    screen.blit(dice2_cpu.my_dice, (21, 90))
    ...
    if show_msg_player1:
        screen.blit(no_move_possible, (19, 818))
        pg.time.delay(3000)
        show_msg_player1 = None
    if show_msg_player2:
        screen.blit(no_move_possible, (746, 818))
        pg.time.delay(3000)
