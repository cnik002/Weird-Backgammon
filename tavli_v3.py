...
active_col = -1   # variable specifying selected column
...
class puck:
    ...
    def get_t_coords(self, color, sz):
        if color == "black": 
            ...
            self.y_coord = black_tower.ypos - 5 + col_height - sz * puck_h
        else:
...
class column:
    ...
    def add_piece(self, piece_to_add): #pushing
        # if move is performed then validity was already checked -> make col color  = puck color
...
def move(FROM, TO):
    ...
    if TO.number == 30 or TO.number == 60:
        ...
        # change 1 - to add was 1st line in if and else
...
def seat_white():
        ...
        if dice.d1 > 0 and (columns[dice.d1].color == "white" or 0 <= columns[dice.d1].size <= 1):
            ...
            d1ok = False
        # change 2 - doubles check and if for d2 became elif
        if dice.doubles:
            d2ok = d1ok
        # same for d2
        elif not dice.doubles and dice.d2 > 0 and (columns[dice.d2].color == "white" or 0 <= columns[dice.d2].size <= 1):
            moves.append(columns[dice.d2])
            d2ok = True
        else:
            d2ok = False
        # same for d1 + d2 if only 1 puck is hit
        # change 3 - updated d1+d2
...
# change 4 - make same as white if that works
def seat_black():
        # if dice.doubles:
        ...
        if dice.doubles:
            d2ok = d1ok
        # same for d2
        elif not dice.doubles and dice.d2 > 0 and (columns[25-dice.d2].color == "black" or 0 <= columns[25-dice.d2].size <= 1):
...
def update_moves(dist, col_flag):
    if not dice.doubles:
        ...
        # change 5 - add more checks in this if block - CHECK THIS IF LOGIC SEEMS WRONG
        print("update moves: moves[0] = ", dist) # debug
    ...
    # change 9 - ad ifs to call legal moves
    if not col_flag:
        legal_moves(active_col)
    elif current_turn == "white":
        legal_moves(25-dist)
    else: legal_moves(dist)   
...
while running:
    ...
    # a player is playing
        ...
        # listen for mouse click on column to show available moves
            ...
            for i in columns:
                # check mouse in playing columns
                # change 6 - shortened this if and added specific checks for collect columns above
                # this is what breaks collecting - working on fix
                if (i.xpos <= mouse[0] <= i.xpos+col_width) and (i.ypos <= mouse[1] <= i.ypos+col_height):
                    active_col = i.number 

                # change 7 - moved this if statement 
                ...
                # change 8 - this if replaced the 3 above commented lines
                # print("before if: ", active_col) # debug
                if 0<=active_col<=25:
                    # print("non collecting range")
                    legal_moves(active_col)
                    show_moves_flag = True
                    temp_col = active_col 
                else: 
                    # print("collect so active col = -1")
                    legal_moves(-1) 
                    show_moves_flag = False
